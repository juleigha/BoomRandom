		...
		<div class="comic-wrapper">
			...
			if( $handle = opendir("Story") ) {
				...
				while(isset($chapters[$chap])) {
					...
					// foreach($files as $file) {
					if ($chap > 6){
						echo "<span class='chapter-titleSpan'><em>{$chapter["name"]}</em></span><img    class='chapter_card' pane-ID='pane{$paneID}' pane-src='/fonts/chaptCard.svg' src='' loading='lazy'><ul class='chapter-ul' id='{$chapter["folder"]}'>";
						$paneID++;
					}
					else {
